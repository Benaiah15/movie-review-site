...
import { Trash2, Loader2, MessageSquare, Star, Film } from "lucide-react";
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-6xl mx-auto pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b dark:border-zinc-800 border-gray-200 pb-6">
        <div>
          ...
          <p className="text-sm dark:text-zinc-500 text-zinc-500 mt-1">Read and moderate user-submitted movie reviews.</p>
        </div>
      ...
      <div className="md:hidden space-y-4">
        {reviews.map(review => (
          <div key={review.id} className="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm flex flex-col gap-3">
            ...
            <p className="text-sm dark:text-zinc-300 text-slate-700 line-clamp-3">{review.content}</p>
          </div>
        ))}
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 dark:bg-zinc-900/50 border-b border-slate-200 dark:border-zinc-800 text-xs uppercase tracking-wider font-bold text-slate-500 dark:text-zinc-500">
              ...
              <th className="p-4">Movie & Rating</th>
              <th className="p-4 w-1/2">Review Content</th>
              <th className="p-4 pr-6 text-right">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/50">
            {reviews.map(review => (
              <tr key={`desktop-${review.id}`} className="hover:bg-slate-50 dark:hover:bg-zinc-900/30 transition-colors group">
                ...
                <td className="p-4">
                  <div className="flex flex-col gap-1">
                    <span className="text-sm font-semibold dark:text-zinc-300 text-slate-700 truncate max-w-[200px]">{review.movie.title}</span>
                ...
                <td className="p-4">
                  <p className="text-sm dark:text-zinc-400 text-slate-600 line-clamp-2" title={review.content}>
                    {review.content}
                  </p>
                </td>
