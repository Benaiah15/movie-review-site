...
export default async function ProfilePage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const user = await db.user.findUnique({
    ...
    include: {
      followers: true, 
      following: true, 
      topFourMovies: true, // <-- NEW DATABASE PULL
  ...
  return (
    <div className="min-h-screen dark:bg-zinc-950 bg-gray-50 pb-20 pt-8 px-4 md:px-8 transition-colors duration-300">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 mt-4 md:mt-12">
        
        {/* ================= LEFT SIDEBAR ================= */}
        <aside className="w-full md:w-72 flex-shrink-0 space-y-6">
          <div className="dark:bg-zinc-900/50 bg-white border dark:border-zinc-800 border-gray-200 shadow-sm rounded-2xl p-6 flex flex-col items-center text-center relative overflow-hidden transition-colors">
        ...
        <main className="flex-1 space-y-8">
            ...
            <div className="animate-in fade-in duration-500">
              ...
              <div className="mb-12">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-6">
                  {/* Create exactly 4 slots using array mapping */}
                  {[0, 1, 2, 3].map((index) => {
                        ...
                        {movie ? (
                          <Link href={`/movie/${movie.id}`} className="block w-full h-full">
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                              <span className="text-white font-bold text-sm truncate drop-shadow-md">{movie.title}</span>
                            </div>
                          </Link>
                        ) : (
                          ...
                          <Link href="/movies" className="flex flex-col items-center justify-center w-full h-full border-2 border-dashed dark:border-zinc-800 border-gray-300 hover:border-pink-500/50 hover:bg-pink-50 dark:hover:bg-pink-500/5 transition-all dark:text-zinc-600 text-zinc-400 hover:text-pink-500 cursor-pointer">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-widest text-center px-4">Pick a<br/>Favorite</span>
                          </Link>
                        )}
                      </div>
                    );
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
                <div className="flex flex-col justify-center dark:bg-zinc-900/50 bg-white border dark:border-zinc-800 border-gray-200 p-5 rounded-2xl shadow-sm transition-colors">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p className="text-xs dark:text-zinc-500 text-zinc-500 font-bold uppercase tracking-wider transition-colors">Reviews</p>
                  </div>
                ...
                <div className="flex flex-col justify-center dark:bg-zinc-900/50 bg-white border dark:border-zinc-800 border-gray-200 p-5 rounded-2xl shadow-sm transition-colors">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p className="text-xs dark:text-zinc-500 text-zinc-500 font-bold uppercase tracking-wider transition-colors">Upvotes</p>
                  </div>
                ...
                <div className="flex flex-col justify-center dark:bg-zinc-900/50 bg-white border dark:border-zinc-800 border-gray-200 p-5 rounded-2xl col-span-2 sm:col-span-1 shadow-sm transition-colors">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p className="text-xs dark:text-zinc-500 text-zinc-500 font-bold uppercase tracking-wider transition-colors">Joined</p>
                  </div>
              ...
              {totalReviews > 0 && (
                <div className="dark:bg-zinc-900/30 bg-white border dark:border-zinc-800/50 border-gray-200 rounded-2xl p-6 md:p-8 mb-8 shadow-sm transition-colors">
                  ...
                  <div className="mt-4 pt-4 border-t dark:border-zinc-800/50 border-gray-100 flex items-center justify-center gap-1 text-sm dark:text-zinc-500 text-zinc-500 font-medium transition-colors">
                    <Star size={14} className="text-amber-500 fill-amber-500" />
                    Based on {totalReviews} rated movies
                  </div>
                </div>
              )}
              ...
              <div className="dark:bg-zinc-900/30 bg-white border dark:border-zinc-800/50 border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm transition-colors">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h2 className="text-2xl font-bold dark:text-white text-zinc-900 tracking-tight transition-colors">Review Diary</h2>
                </div>

                <div className="space-y-4">
                  {user.reviews.length === 0 ? (
                    <div className="text-center py-12 border border-dashed dark:border-zinc-800 border-gray-300 rounded-xl transition-colors">
                      ...
                      <div key={review.id} className="dark:bg-zinc-950 bg-gray-50 border dark:border-zinc-800 border-gray-200 rounded-xl p-4 md:p-5 flex flex-col sm:flex-row gap-5 dark:hover:border-zinc-700 hover:border-gray-300 transition-colors">
                        ...
                        <div className="flex-1 flex flex-col">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <Link href={`/movie/${review.movie.id}`} className="text-lg font-bold dark:text-white text-zinc-900 hover:text-red-500 transition-colors">{review.movie.title}</Link>
          ...
          {currentTab === "watchlist" && (
            <div className="dark:bg-zinc-900/30 bg-white border dark:border-zinc-800/50 border-gray-200 rounded-2xl p-6 md:p-8 animate-in fade-in duration-500 shadow-sm transition-colors">
              <div className="flex items-center gap-3 mb-8">
                <div className="w-1.5 h-6 bg-amber-500 rounded-sm"></div>
                <h2 className="text-2xl font-bold dark:text-white text-zinc-900 tracking-tight transition-colors">Your Watchlist</h2>
              </div>

              {user.favoriteMovies.length === 0 ? (
                <div className="text-center py-20 border border-dashed dark:border-zinc-800 border-gray-300 rounded-xl transition-colors">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                  {user.favoriteMovies.map((movie) => (
                    <Link href={`/movie/${movie.id}`} key={movie.id} className="group flex flex-col gap-2">
                      ...
                      <h3 className="dark:text-white text-zinc-900 font-bold text-sm truncate group-hover:text-amber-500 transition-colors">{movie.title}</h3>
                    </Link>
                  ))}
                </div>
              )}
          ...
          {currentTab === "settings" && (
            <div className="dark:bg-zinc-900/30 bg-white border dark:border-zinc-800/50 border-gray-200 rounded-2xl p-6 md:p-8 animate-in fade-in duration-500 shadow-sm transition-colors">
              <div className="flex items-center gap-3 mb-8">
                <div className="w-1.5 h-6 bg-zinc-500 rounded-sm"></div>
                <h2 className="text-2xl font-bold dark:text-white text-zinc-900 tracking-tight transition-colors">Account Settings</h2>
              </div>

              <div className="mb-8 pb-8 border-b dark:border-zinc-800/50 border-gray-200 transition-colors">
