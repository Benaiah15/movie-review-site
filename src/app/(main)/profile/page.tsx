...
import { Star, MessageSquare, Award, Clock, LayoutDashboard, Bookmark, Camera, PlayCircle, Settings, Users, BarChart3, Pin, X } from "lucide-react"; 
...
export default async function ProfilePage({ searchParams }: { searchParams: Promise<{ tab?: string, modal?: string }> }) {
    ...
    <div className="min-h-screen dark:bg-zinc-950 bg-gray-50 pb-20 pt-8 px-4 md:px-8 transition-colors duration-300 overflow-x-hidden relative">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="dark:bg-zinc-900 bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link href="?tab=activity" className="p-2 dark:bg-zinc-800 bg-gray-100 rounded-full hover:bg-gray-200 dark:hover:bg-zinc-700 transition-colors"><X size={16}/></Link>
            </div>
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 mt-4 md:mt-12 w-full">
        <aside className="w-full md:w-72 flex-shrink-0 space-y-6">
          <div className="dark:bg-zinc-900/50 bg-white border dark:border-zinc-800 border-gray-200 shadow-sm rounded-2xl p-6 flex flex-col items-center text-center relative overflow-hidden transition-colors w-full">
             ...
             <div className="flex items-center justify-center gap-6 text-sm dark:text-zinc-400 text-zinc-500 border-t dark:border-zinc-800/50 border-gray-100 pt-4 w-full transition-colors">
               ...
               <Link href="?modal=following" scroll={false} className="flex flex-col items-center group cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800/50 p-2 rounded-xl transition-colors">
        ...
        <main className="flex-1 space-y-8 w-full max-w-full overflow-hidden">
          ...
          {currentTab === "watchlist" && (<div className="dark:bg-zinc-900/30 bg-white border dark:border-zinc-800/50 border-gray-200 rounded-2xl p-6 md:p-8 animate-in fade-in duration-500 shadow-sm transition-colors w-full overflow-hidden">
            ...
            {user.favoriteMovies.length === 0 ? (
              <div className="text-center py-20 border border-dashed dark:border-zinc-800 border-gray-300 rounded-xl transition-colors w-full">
                ...
                <Link href="/movies" className="text-amber-500 hover:text-amber-400 font-semibold transition-colors">Browse the catalog &rarr;</Link>
              </div>
            ) : (
          ...
          {currentTab === "settings" && (<div className="dark:bg-zinc-900/30 bg-white border dark:border-zinc-800/50 border-gray-200 rounded-2xl p-6 md:p-8 animate-in fade-in duration-500 shadow-sm transition-colors w-full overflow-hidden">
            <div className="flex items-center gap-3 mb-8">
